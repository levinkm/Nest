...
class FinancialProjectionsCalculator {
  static Future<FinancialProjections> calculate(
    ...
    final debtToIncomeRatio = monthlyIncome > 0
        ? (stats.totalDebt / monthlyIncome)
        : 0.0;

    // Max borrowing capacity (30% of monthly income rule)
    final maxMonthlyDebtPayment = monthlyIncome * 0.3;
    final currentMonthlyDebtPayment =
        stats.totalDebt * 0.1; // Assume 10% monthly repayment
    final maxBorrowingCapacity =
        (maxMonthlyDebtPayment - currentMonthlyDebtPayment) * 10;

    // Emergency fund (3-6 months of expenses)
    final emergencyFundNeeded = monthlyExpense * 3;

    // Savings target (20% of income after debt)
    final monthlySavingsTarget =
        (monthlyIncome - monthlyExpense - currentMonthlyDebtPayment) * 0.2;
