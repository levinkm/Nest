...
class SavingsAnalytics {
  static Future<SavingsMetrics> calculate(
    ...
    for (var t in transactions) {
      if (t.date.isAfter(thirtyDaysAgo)) {
        if (t.type.toLowerCase().contains('income') ||
            t.type.toLowerCase().contains('received')) {
  ...
  static List<GoalAnalytic> _analyzeGoals(List<Map<String, dynamic>> goals) {
    ...
    for (var goal in goals) {
      ...
      if (isRecurring && recurringAmount > 0) {
        ...
        projectedCompletion = DateTime.now().add(
  ...
  static List<String> _generateSuggestions(
    ...
    if (emergencyFund < emergencyTarget * 0.5) {
      suggestions.add(
        '🚨 Priority: Build emergency fund to at least 50% (${((emergencyTarget * 0.5 - emergencyFund)).toStringAsFixed(0)} more needed)',
      );
    }
    ...
    if (savingsRate < 10) {
      suggestions.add(
        '📈 Increase savings rate to at least 10% (currently ${savingsRate.toStringAsFixed(1)}%)',
      );
    } else if (savingsRate < 20) {
      suggestions.add(
        '💪 Good progress! Aim for 20% savings rate (currently ${savingsRate.toStringAsFixed(1)}%)',
      );
    } else {
      suggestions.add(
        '🌟 Excellent! Your ${savingsRate.toStringAsFixed(1)}% savings rate exceeds the 20% goal',
      );
    }
    ...
    if (potentialSavings > 0) {
      suggestions.add(
        '💰 You can save ${potentialSavings.toStringAsFixed(0)} more this month',
      );
    }
    ...
    for (var goal in goals) {
      ...
      if (daysLeft > 0 && daysLeft < 30 && current < target) {
        final remaining = target - current;
        suggestions.add(
          '⏰ Goal "${goal['name']}" due in $daysLeft days. Need ${remaining.toStringAsFixed(0)} more',
        );
      }
    }
    ...
    if (!hasRecurring && goals.isNotEmpty) {
      suggestions.add(
        '🤖 Enable recurring savings on your goals for automatic progress',
      );
    }
