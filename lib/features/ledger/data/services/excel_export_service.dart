...
class ExcelExportService {
  static Future<File> exportLedgerToCSV(
    ...
    buffer.writeln(
      'Total Credits,${currencyFormat.format(summary['totalCredits'])}',
    );
    buffer.writeln(
      'Total Debits,${currencyFormat.format(summary['totalDebits'])}',
    );
    buffer.writeln('Total Fees,${currencyFormat.format(summary['totalFees'])}');
    buffer.writeln(
      'Net Balance,${currencyFormat.format(summary['netBalance'])}',
    );
    ...
    final fileName =
        '${accountName.replaceAll(' ', '_')}_Ledger_$timestamp.csv';
      ...
      for (final ledgerEntry in entries) {
        buffer.writeln(
          [
            ...
            ledgerEntry.debit > 0
                ? '"${ledgerEntry.debit.toStringAsFixed(2)}"'
                : '',
            ledgerEntry.credit > 0
                ? '"${ledgerEntry.credit.toStringAsFixed(2)}"'
                : '',
            ledgerEntry.fee > 0
                ? '"${ledgerEntry.fee.toStringAsFixed(2)}"'
                : '',
