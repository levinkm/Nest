...
class SmartBudgetService {
  ...
  double suggestBudget(String category, List<Transaction> history) {
    final categoryTxns = history.where((t) => 
  ...
  int calculateBudgetHealth(List<Budget> budgets, double income) {
    ...
    final adherence = activeBudgets.where((b) => !b.isOverBudget).length / activeBudgets.length;
    
    // Savings rate
    final totalSpent = activeBudgets.fold(0.0, (sum, b) => sum + b.spent);
    final savingsRate = income > 0 ? ((income - totalSpent) / income).clamp(0.0, 1.0) : 0;
    
    // Coverage: % of spending that's budgeted
    final totalBudgeted = activeBudgets.fold(0.0, (sum, b) => sum + b.amount);
    final coverage = totalSpent > 0 ? (totalBudgeted / totalSpent).clamp(0.0, 1.0) : 1.0;
  ...
  List<String> generateInsights(Budget budget, List<Transaction> transactions) {
    ...
    if (budget.paceStatus == 'fast') {
      ...
      insights.add('⚠️ Spending 20% faster than planned. May exceed by KSh ${overage.toStringAsFixed(0)}');
    }
    ...
    if (weekendTxns.isNotEmpty) {
      ...
      if (weekendSpent > weekdaySpent * 0.5) {
        insights.add('📊 You spend ${((weekendSpent / budget.spent) * 100).toStringAsFixed(0)}% on weekends');
      }
    }
  ...
  double calculateAverageSpending(String category, List<Transaction> history, int months) {
  ...
  Map<String, double> detectMissingBudgets(
    ...
    final budgetedCategories = existingBudgets.map((b) => b.category?.toLowerCase()).toSet();
  ...
  Map<String, dynamic> generateMonthlyReport(Budget budget, List<Transaction> transactions) {
