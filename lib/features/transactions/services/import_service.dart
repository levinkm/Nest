...
class ImportService {
  ...
  List<Transaction> _parsePdfText(String text) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      final receiptMatch = RegExp(r'([A-Z0-9]{10,})\s+(\d{1,2}/\d{1,2}/\d{2,4})').firstMatch(line);
      ...
      final detailsMatch = RegExp(r'(?:Paid to|Received from|Sent to|Withdraw from)\s+([^0-9]+)').firstMatch(line);
      ...
      final type = line.contains('Paid to') || line.contains('Sent to') || line.contains('Withdraw') 
          ? 'expense' 
          : 'income';
  ...
  String _categorizeFromDescription(String description, String type) {
    ...
    if (lower.contains('supermarket') || lower.contains('shop')) return 'Groceries';
    ...
    if (lower.contains('electricity') || lower.contains('water') || lower.contains('rent')) return 'Bills';
    if (lower.contains('airtime') || lower.contains('data')) return 'Airtime & Data';
    if (lower.contains('withdraw') || lower.contains('atm')) return 'Cash Withdrawal';
